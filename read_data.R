...
# Prediction
X.pred <- DL.hstep(X, q, Phi.hat, gamma.hat, h)

# Plot prediction
plot(1:n,X, type='l', xlim=c(0,2800))
lines((n+1):(n+h),X.pred, type='l', col="red")
...
plot(1:n, X, type="l",xlim=c(1, n+h))
lines((n+1):(n+h), exp(X.pred[(n+1):(n+h)]))
